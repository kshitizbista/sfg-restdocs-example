...
@AutoConfigureRestDocs(uriScheme = "https", uriHost = "dev.springframework.guru", uriPort = 8080)   //customizing uri
...
class BeerControllerTest {
    ...
    void getBeerById() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/beer/{beerId}", UUID.randomUUID().toString()).accept(MediaType.APPLICATION_JSON)
                .param("isCold", "yes")) // for demonstration purpose only (documenting request param, it will get ignored when no param is not set in controller)
                .andExpect(status().isOk())
                .andDo(document("v1/beer-get", //folder location inside target/generated-snippets (should be unique for every test)
                        pathParameters(parameterWithName("beerId")
                                .description("UUID of desired beer to get.")),
                        requestParameters(parameterWithName("isCold") // for demonstration purpose only (documenting request param)
